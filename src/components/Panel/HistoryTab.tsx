...
import { ChefHat, Star, History, Info, AlertCircle, CheckCircle2, RefreshCw, Send, Camera } from 'lucide-react';
...
export default function HistoryTab({ mealHistory, onViewDetails, onEvaluate, onRefresh }: HistoryTabProps) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-end mb-12">
        <div>
          <div className="text-[10px] font-mono text-primary tracking-[0.5em] uppercase mb-2">BIO_LOG_V3.0</div>
          <h3 className="text-4xl font-display font-black uppercase text-white tracking-tighter">Historial</h3>
        </div>
        <div className="flex gap-4">
          <button 
            onClick={onRefresh}
            className="p-3 bg-white/5 border border-white/10 rounded-2xl text-gray-500 hover:text-primary transition-all group"
          >
            <RefreshCw size={20} className="group-hover:rotate-180 transition-transform duration-500" />
          </button>
          <div className="px-8 py-3 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-xl">
            <div className="text-[8px] font-mono text-gray-500 uppercase tracking-[0.3em] mb-1">REGISTROS</div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {mealHistory.length === 0 ? (
          <div className="card-cyber p-20 text-center">
             ...
             <p className="text-gray-500 font-mono text-sm tracking-widest uppercase italic">Esperando sincronización de datos biométricos...</p>
          </div>
        ) : (
          mealHistory.map((item) => {
            const isPending = !item.rating || item.rating === 0;
            const isExpanding = expandingId === item.id;

            return (
              <div key={item.id} className={`card-cyber p-0 overflow-hidden group transition-all duration-500 ${isPending ? 'border-yellow-500/30 bg-yellow-500/5' : 'hover:border-primary/50'}`}>
                  ...
                  <div className="flex items-center gap-8 flex-1 w-full md:w-auto">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {isPending ? (
                          <span className="px-2 py-0.5 bg-yellow-500/10 border border-yellow-500/30 rounded text-[8px] font-mono text-yellow-500 uppercase animate-pulse">Pendiente_Feedback</span>
                        ) : (
                          <span className="px-2 py-0.5 bg-green-500/10 border border-green-500/30 rounded text-[8px] font-mono text-green-500 uppercase">Sincronizado</span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-6">
                        {!isPending ? (
                          ...
                          >
                            <Send size={14} /> Enviar Bio-Feedback
                          </button>
                        )}
                      </div>
                    </div>

...
import { i18n, sizes } from '../data';
import HeroBattleSlider from '../components/HeroBattleSlider';
import MealCard from '../components/MealCard';

export default function Demo({ lang, toggleLang, goTo, user, onLogout, setShowLogin, setIsRegistering, selectedDays, setSelectedDays }: any) {
    ...
    fetch('/api/meals/list')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          console.error("API_FORMAT_ERROR: Expected array, got", data);
          setMeals([]);
        }
      })
      .catch(err => {
        console.error("API_LOAD_ERROR:", err);
        setMeals([]);
      });

    const categoryOrder = ['breakfast', 'lunch', 'dinner', 'snack', 'juices'];
    const interval = setInterval(() => {
      setActiveCategory(prev => {
        const idx = categoryOrder.indexOf(prev);
        return categoryOrder[(idx + 1) % categoryOrder.length];
      });
      setShowAll(false);
    }, 5000);
  ...
  const filteredMeals = Array.isArray(meals) ? meals.filter(m => m.category === activeCategory) : [];
  ...
  return (
    <div className="relative overflow-hidden bg-black text-white selection:bg-primary selection:text-black">
      ...
      <section className="relative pt-40 pb-20 px-6 text-center max-w-7xl mx-auto overflow-visible">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="z-10 relative">
          ...
          <p className="max-w-3xl mx-auto text-gray-400 font-sans text-lg md:text-xl mb-12 leading-relaxed">{t.hero.desc}</p>
          <button onClick={() => {setIsRegistering(true); setShowLogin(true)}} className="btn-cyber-primary text-md px-16 py-6 flex items-center gap-3 mx-auto">
      ...
      <section id="savings" className="py-24 px-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {t.savings.items.map((item: any, i: number) => (
            <div key={i} className="card-cyber p-8 text-center flex flex-col items-center group bg-gradient-to-b from-white/5 to-transparent">
              <div className="w-16 h-16 rounded-2xl bg-black border border-white/10 flex items-center justify-center text-primary mb-6 transition-all group-hover:border-primary">
                {i === 0 && <Timer size={32} />}
                {i === 1 && <Flame size={32} />}
      ...
      <section id="menu" className="py-32 px-6 max-w-7xl mx-auto border-t border-white/5">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeCategory} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {visibleMeals.map((meal, i) => (
              <MealCard 
                key={meal.id_code} 
                meal={meal} 
                lang={lang} 
                sizeMultiplier={sizes[activeSize].multiplier} 
              />
            ))}
          </motion.div>
        </AnimatePresence>
      ...
      <footer className="py-20 px-6 border-t border-white/10 bg-white/2 overflow-hidden relative text-center">
      ...
      <style>{`
        ...
        .glitch-text::after { content: attr(data-text); position: absolute; left: 0; top: 0; width: 100%; height: 100%; background: black; clip-path: polygon(0 0, 100% 0, 100% 45%, 0 45%); transform: translate(-2px, -2px); opacity: 0.8; animation: glitch 2s infinite; }
